...
import test.NucleotideFrequency;
...
//    private int progress=0;
    ...
    public RecFinder(Alignment aln, double[][] mfes, int window){
        this.aln=aln;
    ...
    public void oneTripletScan(int rec, int p1, int p2){
        double[] pd1=new double[mfes.length];
        double[] pd2=new double[mfes.length];
        double[] pd3=new double[mfes.length];
        fillPdArray(pd1,rec,p1);
        fillPdArray(pd2,rec,p2);
        fillPdArray(pd3,p1,p2);
        Recombinant r=new Recombinant(rec);
        ...
        double[] spdd=Loess.smooth(pdd);
        
        for(int i=0;i<mfes.length;i++)
            System.out.println(i+"\t"+pdd[i]+"\t"+spdd[i]);
        
        ArrayList<Integer> cps=crossPointsOf(spdd);
